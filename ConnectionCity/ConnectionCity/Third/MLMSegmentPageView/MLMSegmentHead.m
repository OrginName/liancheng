...
- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles headStyle:(MLMSegmentHeadStyle)style {
    if (self = [super initWithFrame:frame]) {
        _headStyle = style;
        titlesArray = titles;
//        self.layer.shadowColor = YSColor(231, 231, 231).CGColor;
//        self.layer.shadowOpacity = 0.8f;
        [self SetShadowPathWith:YSColor(231, 231, 231) shadowOpacity:0.3 shadowRadius:2 shadowSide:ShadowPathBottom shadowPathWidth:10];
        [self initCustom];
    }
    return self;
}
